...
class Data_preparation(object):
    def __init__(self, data_path, img_dir, im_metadata_path,
                 im_dim=224, output_dim=14, num_images=None,
                 max_rels_per_image=None):
        """Constructor for the dataset object.

        Args:
            data_path: Annotations in the dataset.
            (vrd_eg:"5531467160_2ae2cb76d4_b.jpg": 
            [{"predicate": 0, "object": {"category": 59, "bbox": [357, 481, 819, 854]}, 
            "subject": {"category": 25, "bbox": [271, 355, 788, 873]}}) Note:bbox:[YMIN, YMAX, XMIN, XMAX])

            img_dir: Location of the images.

            im_metadata_path: Location of the file containing image metadata.
            (vrd_eg:"5531467160_2ae2cb76d4_b.jpg": {"width": 1024, "height": 768})
        ...
        self.data = data
        #self.data = dict(sorted(data.items(), key=lambda x: x[0])[:1000])
    ...
    def get_image_from_img_id(self, img_id):
        ...
        img = img.resize((self.im_dim, self.im_dim),Image.ANTIALIAS)
        if img.mode != 'RGB':
            img = img.convert('RGB')
    ...
    def get_regions_from_bbox(self, bbox):
        ...
        result = (col_indexes * row_indexes).reshape((self.output_dim, self.output_dim))
        return result
...
class Build_dataset(Data_preparation):
    """
